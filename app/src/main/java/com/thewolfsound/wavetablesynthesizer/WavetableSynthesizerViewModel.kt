...
//  private var wavetable = Wavetable.SINE
  ...
  private var oscNum = 2
  private var minFreq: Float = 400.0f
  private var maxFreq: Float = 1000.0f

  fun setBufferSize(newBufferSize: Int) {
    bufferSize = newBufferSize
    viewModelScope.launch {
      wavetableSynthesizer?.setBufferSize(newBufferSize)
    }
  }

  fun setOscNum(newOscNum: Int) {
    oscNum = newOscNum
    viewModelScope.launch {
      wavetableSynthesizer?.setOscNum(newOscNum)
    }
  }

  fun setMinFrequency(newMinFreq: Float) {
    minFreq = newMinFreq
    viewModelScope.launch {
      wavetableSynthesizer?.setMinFrequency(newMinFreq)
    }
  }

  fun setMaxFrequency(newMaxFreq: Float) {
    maxFreq = newMaxFreq
    viewModelScope.launch {
      wavetableSynthesizer?.setMaxFrequency(newMaxFreq)
    }
  }
  /**
